...
from l5kit.data.proto.road_network_pb2 import (
    GeoFrame,
    GlobalId,
    MapElement,
    MapFragment,
...
class MapAPI:
    def __init__(self, protobuf_map_path: str, world_to_ecef: np.ndarray):
        ...
        self.ids_to_el = {
            self.id_as_str(el.id): idx for idx, el in enumerate(self.elements)
        }  # store a look-up table

        self.lane_orientation_code_2_str = {
            0: "UNKNOWN_TRAVEL_DIRECTION",
            1: "TWO_WAY",
            2: "ONE_WAY_FORWARD",
            3: "ONE_WAY_BACKWARD",
    ...
    def get_all_lanes_at_junction(self, junction_element_id: str) -> list:
        ...
        road_network_nodes = self.get_road_network_nodes_at_junction(
            junction_element_id
        )
        all_road_segments = [
            x
        ...
        all_lanes_assigned_to_segments = [
            x
    ...
    def get_lane_to_left(self, element_id: str) -> str:
        ...
        lane = element.element.lane
        return (
            self.id_as_str(lane.adjacent_lane_change_left)
            if lane.HasField("adjacent_lane_change_left")
            and self.id_as_str(lane.adjacent_lane_change_left) != "0"
            else ""
        )
    ...
    def get_lane_to_right(self, element_id: str) -> str:
        ...
        lane = element.element.lane
        return (
            self.id_as_str(lane.adjacent_lane_change_right)
            if lane.HasField("adjacent_lane_change_right")
            and self.id_as_str(lane.adjacent_lane_change_right) != "0"
            else ""
        )
    ...
    def is_crosswalk(element: MapElement) -> bool:
        ...
        return bool(
            traffic_element.HasField("pedestrian_crosswalk")
            and traffic_element.points_x_deltas_cm
        )
    ...
    def get_traffic_light_face_sets(self, element_id):
        ...
        for colour in ["red", "green", "yellow"]:
            for face_set_name in [
                f"signal_{colour}_face",
    ...
    def has_lane_stop_sign(self, lane_id):
        ...
        for traffic_control in traffic_controls:
            ...
            traffic_control_element = self[
                traffic_control_el_id
            ].element.traffic_control_element
    ...
    def get_rules_for_traffic_light_face_set(traffic_light_face_set):
        ...
        for yield_rules_when_on_ in traffic_light_face_set.yield_rules_when_on:
            ...
            result.append(
                {
                    "lane": lane,
                    "yield_to_lanes": yield_to_lanes,
                    "yield_to_crosswalks": yield_to_crosswalks,
                }
            )
        return result
