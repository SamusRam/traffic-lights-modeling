...
if not perform_prediction:
    ...
    with open(f'../outputs/tl_predict_checkpoints/intersection_{intersection_idx}_fold_{fold_i}_train_vocab.pkl',
              'wb') as f:
        pickle.dump(train_vocab, f)
    with open(f'../outputs/tl_predict_checkpoints/intersection_{intersection_idx}_fold_{fold_i}_term_freq.pkl',
              'wb') as f:
        pickle.dump(term_freq, f)
else:
    model_fold_i = int((int(fold_i) + 1) % 2)
    with open(f'../outputs/tl_predict_checkpoints/intersection_{intersection_idx}_fold_{model_fold_i}_train_vocab.pkl',
              'rb') as f:
        train_vocab = pickle.load(f)
    with open(f'../outputs/tl_predict_checkpoints/intersection_{intersection_idx}_fold_{model_fold_i}_term_freq.pkl',
              'rb') as f:
...
class IntersectionModel(nn.Module):
    ...
    def get_weibull_quantiles(self, weibull_lambda, weibull_k, quantile):
        return weibull_lambda * torch.pow(-torch.log(torch.tensor([1.0 - quantile]).to(self.device)),
    ...
    def get_weibull_mode(self, weibull_lambda, weibull_k):
        ...
        mode = weibull_lambda * torch.pow((weibull_k - 1.0) / weibull_k, 1.0 / weibull_k)
        mode[weibull_k <= 1.0] = 0.0
        return mode

    def forward(self, tokens_seq, token_type_ohe, token_timesteps, seq_lengths, output_mode_with_percentiles=False):
        ...
        for tl_id in self.intersection_tl_signals:
            ...
            if output_mode_with_percentiles:
                ...
                tte_25th_percentile = self.get_weibull_quantiles(weibull_lambda, weibull_k, 0.25)
                tte_75th_percentile = self.get_weibull_quantiles(weibull_lambda, weibull_k, 0.75)
                tl_2_tte_mode_quantiles[tl_id] = torch.cat((tte_mode,
                                                            tte_25th_percentile,
                                                            tte_75th_percentile), dim=1)
...
class IntersectionDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        if self.return_indices:
            return (tokens_np, token_type_ohe_np, token_timesteps_np, seq_len,
                    all_true_classes, all_tte, classes_availabilities, tte_availabilities,
                    self.tl_events_df['scene_idx'].iloc[row_i], self.tl_events_df['frame_idx'].iloc[row_i])
...
def get_valid_indices(tl_events_df, history_len_records=HIST_LEN_FRAMES, is_inference=False):
    ...
    is_nonempty_input_present_series = is_nonempty___is_continuious_series.rolling(history_len_records - 1,
                                                                                   min_periods=1).agg(is_nonempty_input_present)
    
    valid_rows_bool = (tl_events_df['tl_signal_classes'].map(lambda x: len(x) > 0 or is_inference) &
...
def get_dataloader(tl_events_df, intersection_idx, shuffle=True, train_vocab=train_vocab,
                   ...
                   return_indices=False, is_inference=False):
    ...
    valid_indices_intersection = get_valid_indices(tl_events_df_intersection, is_inference=is_inference)
...
def predict(dataloader, intersection_model, device,
            eval_performance=True, binary_crossentropy=nn.BCELoss(reduction="none")):
    progress_bar = tqdm((dataloader), desc=f'Prediction..')
    intersection_model.eval()
    if eval_performance:
        losses = []
        losses_lob_prob = []
    ...
    traffic_light_indices = [int(name.replace('fc_tte_k_', ''))
    ...
    for batch in progress_bar:
        (tokens, token_type_ohe, token_timesteps, seq_len,
         all_true_classes, all_tte, classes_availabilities, tte_availabilities,
         scene_indices, frame_indices) = batch
        # moving to GPU if available
        tokens, token_type_ohe, token_timesteps, seq_len = tokens.to(device), token_type_ohe.to(
            device), token_timesteps.to(device), seq_len.to(device)
        tl_2_color_class, tl_2_tte_distr, tl_2_tte_mode_quantiles = intersection_model(tokens, token_type_ohe,
                                                                                       token_timesteps, seq_len,
                                                                                       output_mode_with_percentiles=True)

        scene_indices_list.extend(scene_indices.numpy() if device == 'cpu' else scene_indices.cpu().numpy())
        frame_indices_list.extend(frame_indices.numpy() if device == 'cpu' else frame_indices.cpu().numpy())
        tl_idx_2_tte_preds = [(key, val.detach().numpy() if device == 'cpu' else val.detach().cpu().numpy())
        ...
        for tl_id, green_color_prob_torch in tl_2_color_class.items():
            tl_idx_2_green_color_prob_list[tl_id].extend(green_color_prob_torch.detach().numpy().reshape(-1)
                                                         if device == 'cpu' else green_color_prob_torch.detach().cpu().numpy().reshape(-1)  )
        ...
        if eval_performance:
            ...
            for tl_id, pred_color_classes in tl_2_color_class.items():
                true_color_classes = all_true_classes[tl_id]
                bce_loss_tl = binary_crossentropy(torch.squeeze(pred_color_classes), true_color_classes) * \
    ...
    values_dict = {'scene_idx': scene_indices_list,
                   'scene_frame_idx': frame_indices_list}
    for tl_id in traffic_light_indices:
        values_dict.update({f'{tl_id}_green_prob': tl_idx_2_green_color_prob_list[tl_id],
