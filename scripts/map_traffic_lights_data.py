...
def get_lanes_dict_and_id_mapping(filter_function):
    ...
    for key, values in lanes_crosswalks['lanes'].items():
        if len(lanes_crosswalks['lanes']['ids']) == len(values):
            lanes[key] = values[lanes_indices] if isinstance(values, (np.ndarray, np.generic)) else [values[i] for i in
                                                                                                     lanes_indices]
...
lane_adj_list_forward_bike, lane_adj_list_backward_bike, lane_adj_list_right_bike, lane_adj_list_left_bike = precompute_lane_adjacencies(
    lane_id_2_idx_bike, proto_API)
lane_adj_list_forward_not_bike, lane_adj_list_backward_not_bike, lane_adj_list_right_not_bike, lane_adj_list_left_not_bike = precompute_lane_adjacencies(
    lane_id_2_idx_not_bike, proto_API)
...
for fix_i, turn_under_red_fix in enumerate(turn_under_red_fixes):
    ...
    lane_adj_list_forward_specialized[lane_spec][lane_id_2_idx_specialized[lane_spec][parent]] = [f'lane{2 * fix_i}',
                                                                                                  f'lane{2 * fix_i + 1}']

    for lane_id_left in lanes_active:
        lane_adj_list_backward_specialized[lane_spec][lane_id_2_idx_specialized[lane_spec][lane_id_left]] = [
                                                                                                                f'lane{2 * fix_i + 1}'] + [
                                                                                                                x for x
                                                                                                                in
                                                                                                                lane_adj_list_backward_specialized[
                                                                                                                    lane_spec][
                                                                                                                    lane_id_2_idx_specialized[
                                                                                                                        lane_spec][
                                                                                                                        lane_id_left]]
                                                                                                                if
                                                                                                                x != parent]
    lane_adj_list_backward_specialized[lane_spec][lane_id_2_idx_specialized[lane_spec][lane_to_remove]] = [
                                                                                                              f'lane{2 * fix_i}'] + [
                                                                                                              x for x in
                                                                                                              lane_adj_list_backward_specialized[
                                                                                                                  lane_spec][
    ...
    lanes_specialized[lane_spec]['center_line'][lane_id_2_idx_specialized[lane_spec][parent]] = parent_center_line[
                                                                                                :max(1, len(
                                                                                                    parent_center_line) - 3)]
    lanes_crosswalks['lanes']['center_line'][lane_id_2_idx[parent]] = parent_center_line[
                                                                      :max(1, len(parent_center_line) - 3)]
    new_parent_last_coord = lanes_specialized[lane_spec]['center_line'][lane_id_2_idx_specialized[lane_spec][parent]][
        -1].copy()
    ...
    directional_point_for_fork = \
        lanes_specialized[lane_spec]['center_line'][lane_id_2_idx_specialized[lane_spec][lane_to_remove]][3]
    ...
    directional_point_for_fork = \
        lanes_specialized[lane_spec]['center_line'][lane_id_2_idx_specialized[lane_spec][lanes_active[0]]][4]
    new_parent_last_coord = lanes_specialized[lane_spec]['center_line'][lane_id_2_idx_specialized[lane_spec][parent]][
        -1].copy()
...
map_segment_2_kd_tree_bike, map_segment_2_kd_idx_2_lane_id_idx_bike = [None for _ in range(n_map_segments)], [None for _
                                                                                                              in range(
        n_map_segments)]
map_segment_2_kd_tree_not_bike, map_segment_2_kd_idx_2_lane_id_idx_not_bike = [None for _ in range(n_map_segments)], [
    None for _ in range(n_map_segments)]
map_segment_2_kd_tree, map_segment_2_kd_idx_2_lane_id_idx = [None for _ in range(n_map_segments)], [None for _ in range(
    n_map_segments)]
...
for map_segment_idx, segment_lanes in enumerate(map_segment_2_lanes):
    segment_lanes_not_bike_dict, _ = get_lanes_dict_and_id_mapping(
        filter_function=lambda x: x in segment_lanes and
                                  x not in lane_ids_bike_only)
        ...
        for junction in junctions:
            ...
            for junction_lane in junction_2_lanes[junction]:
                if junction_lane in lane_id_2_idx_bike:
                    ...
                    tl_control_2_close_lanes[tl_control].extend(
                        lane_adj_list_backward_not_bike[lane_id_2_idx_not_bike[junction_lane]])
...
def get_lane_point_coordinates(lane_id, point_idx):
    ...
    for tl_id in tls_united_by_lane:
        if tl_id in traffic_light_id_2_master_intersection_idx:
            if master_intersection_idx is not None:
                assert traffic_light_id_2_master_intersection_idx[tl_id] == master_intersection_idx, (f'tl_id: {tl_id}',
...
master_intersection_idx_2_traffic_lights = [set(intersection_tls) for intersection_tls in sorted(
...
def get_lane_boarders(lane_id):
    if lane_id in lane_id_2_idx_bike:
        ...
        return lanes_not_bike['xy_left_'][lane_id_2_idx_not_bike[lane_id]], lanes_not_bike['xy_right_'][
            lane_id_2_idx_not_bike[lane_id]]
    else:
...
for master_intersection_idx, master_intersection_tl_signal_indices in master_intersection_idx_2_tl_signal_indices.items():
        ...
        for lane_id in tl_signal_idx_2_controlled_lanes[tl_signal_idx]:
            ...
            while len(queue):
                ...
                if dist_from_exit < 70:  # points
                    lanes_next = get_lane_successors(next_lane_id) + get_lane_left_neighbours(
                        next_lane_id) + get_lane_right_neighbours(next_lane_id)
    ...
    for lane_id in all_master_intersection_lanes:
        lane_2_master_intersection_related_lanes[lane_id] = {neighbour_lane_id for neighbour_lane_id in
                                                             all_master_intersection_lanes
...
lanes_not_bike_intersection, _ = get_lanes_dict_and_id_mapping(
    filter_function=lambda x: x in added_lanes or (
            not proto_API.is_bike_only_lane(x) and x in lane_id_2_master_intersection_idx))
lanes_bike_intersection, _ = get_lanes_dict_and_id_mapping(
    filter_function=lambda x: x in added_lanes or (
            proto_API.is_bike_only_lane(x) and x in lane_id_2_master_intersection_idx))
...
kd_tree_not_bike_intersection, kd_idx_2_lane_id_idx_not_bike_intersection = get_kd_tree_and_idx_map(
    lanes_not_bike_intersection)
...
def match_point_2_map_segment(x_coord, y_coord,
                              segment_x_coords_only=map_segment_x_coords_only,
                              segment_y_coords_only=map_segment_y_coords_only,
                              interval_2_segments_x=interval_2_map_segments_x,
                              interval_2_segments_y=interval_2_map_segments_y):
...
def get_closest_lanes(coord, agent_class, k_nearest,
    ...
    if agent_class == BIKE_CLASS:
        if intersections_only:
            candidate_distances, candidate_indices = kd_tree_bike_intersection.query(coord, k=k_nearest)
            return candidate_distances, get_candidate_lane_points(candidate_indices, agent_class, intersections_only)
        if segmented_map:
            map_segment_indices = match_point_2_map_segment(*coord, map_segment_x_coords_only,
        ...
        if intersections_only:
            candidate_distances, candidate_indices = kd_tree_not_bike_intersection.query(coord, k=k_nearest)
            return candidate_distances, get_candidate_lane_points(candidate_indices, agent_class, intersections_only)
        if segmented_map:
            map_segment_indices = match_point_2_map_segment(*coord, map_segment_x_coords_only,
                                                            map_segment_y_coords_only,
        ...
        if intersections_only:
            candidate_distances, candidate_indices = kd_tree_intersection.query(coord, k=k_nearest)
            return candidate_distances, get_candidate_lane_points(candidate_indices, agent_class, intersections_only)
        if segmented_map:
            map_segment_indices = match_point_2_map_segment(*coord, map_segment_x_coords_only,
                                                            map_segment_y_coords_only,
            ...
            if len(map_segment_indices):
                # map_segment_indices are ordered based on map segment size
                candidate_distances, candidate_indices = map_segment_2_kd_tree[map_segment_indices[0]].query(
                    coord,
                    k=k_nearest)
                return candidate_distances, get_candidate_lane_points(candidate_indices, agent_class,
                                                                      intersections_only, map_segment_indices[0])
...
def find_closest_lane(coord, yaw, agent_class, lane_point_2_blocked_lanes_set=None,
    ...
    candidate_distances, candidate_lane_points = get_closest_lanes(coord, agent_class, k_nearest, intersections_only)
    ...
    if return_blocked_tl_signals:
        if lane_point_2_blocked_lanes_set is None:
            ...
            while (last_blocked_i < len(candidate_distances) and
                   candidate_distances[last_blocked_i] < candidate_distances[
                       closest_lane_current_i] + blocked_dist_threshold_m):
                last_blocked_i += 1

            blocked_lane_candidates = [lane_id for lane_id, lane_point_i in candidate_lane_points[:last_blocked_i]
...
for i, lane_id in tqdm(enumerate(exit_lane_id_2_tl_signal_idx.keys()), desc='Lane blocked sets..'):
    ...
    for point_idx in range(lane_len - 1, -1, -1):
        ...
        (closest_lane_id, closest_lane_point_i), blocked_tl_signals = find_closest_lane(lane_center_line[point_idx],
                                                                                        yaw,
        ...
        assert (lane_id,
                point_idx) in checked_hard_cases or closest_lane_id == lane_id and point_idx == closest_lane_point_i, f'closest_lane_id: {closest_lane_id}[{closest_lane_point_i}], (true lane_id: {lane_id}[{point_idx}])'
...
def get_info_per_related_lanes(frame_sample):
    ...
    ego_closest_lane_id = find_closest_lane(ego_centroid, ego_yaw, agent_class=ALL_WHEELS_CLASS,
                                            intersections_only=True)
    intersection_related_lanes = lane_2_master_intersection_related_lanes[
        ego_closest_lane_id] if ego_closest_lane_id in lane_2_master_intersection_related_lanes else set()
    ...
    for lane_id, speeds in lane_2_speeds.items():
        lane_results.append((lane_id, np.mean(speeds), np.max(speeds), len(speeds), np.min(lane_2_completions[lane_id]),
    ...
    for current_i in stopped_lane_current_indices:
        ...
        if lane_id in controlled_lane_id_2_tl_signal_idx:
            ...
            signal_2_stopped_car_speed_count[controlling_tl_signal] = (
                max(prev_speed, speed_max), prev_count + car_counts)
...
def compute_tl_signal_classes(tl_events_df):
    tl_signals_buffer = dict()
    timestamp_prev, ego_centroid_prev = datetime(1970, 1, 1).astimezone(timezone('US/Pacific')), np.array(
        [-9999, -9999])
...
def compute_time_to_tl_change(tl_events_df, ego_translation_m_max=1.5, time_to_event_ub=5.01, verbose=False):
    ...
    timestamp_next, ego_centroid_next = datetime(2050, 1, 1).astimezone(timezone('US/Pacific')), np.array(
        [-9999, -9999])
    ...
    for row_i in tqdm(range(len(tl_events_df) - 1, -1, -1), desc='Time to color change..'):
        ...
        if row['master_intersection_idx'] != master_intersection_idx_next:
            ...
            translation_m = np.hypot(ego_centroid_next[0] - row['ego_centroid'][0],
        ...
        for tl_signal_idx, (event_timestamp, event_color_code) in relevant_tl_signal_change_event_items:
            ...
            if event_color_code != row['tl_signal_classes'][tl_signal_idx]:
                time_to_tl_change_current[tl_signal_idx] = np.clip((event_timestamp - row['timestamp']).total_seconds(),
                                                                   0, time_to_event_ub)
            else:
                ...
                if active_next_tl_signals[tl_signal_idx] == row['tl_signal_classes'][tl_signal_idx]:
                    ...
                    elif (the_same_color_duration[tl_signal_idx] - row[
                        'timestamp']).total_seconds() >= time_to_event_ub:
                    ...
                    relevant_tl_signal_change_event[tl_signal_idx] = (
                        timestamp_next, active_next_tl_signals[tl_signal_idx])
                    time_to_tl_change_current[tl_signal_idx] = np.clip(
                        (timestamp_next - row['timestamp']).total_seconds(), 0, time_to_event_ub)
        ...
        timestamp_next, ego_centroid_next, master_intersection_idx_next = row['timestamp'], row['ego_centroid'], row[
...
def get_agent_lanes_info(frame_sample, intersection_2_predictions,
                         min_lane_points_forward=10, max_speed_limit=18):
    ...
    ego_centroid = frame_sample['ego_centroid']
    ego_yaw = frame_sample['ego_yaw']
    ego_closest_lane_id, ego_lane_point_i = find_closest_lane(ego_centroid, ego_yaw, agent_class=ALL_WHEELS_CLASS,
                                                              return_point_i=True)

    lane_2_cars = defaultdict(list)
    ego_speed = np.hypot(*frame_sample['ego_speed']) if frame_sample['ego_speed'] is not None else 0.0
    lane_2_cars[ego_closest_lane_id].append((ego_lane_point_i, ego_centroid, ego_speed, ego_yaw))
    if ego_closest_lane_id in lane_2_master_intersection_related_lanes:
        ...
        for intersection_pred_df in intersection_2_predictions[ego_intersection_i]:
            try:
                tl_events_pred_current_list.append(intersection_pred_df.loc[(scene_idx, state_idx)])
            except KeyError:
                pass
        if len(tl_events_pred_current_list) == 0:
            ...
            tl_events_pred_current = {key: np.nanmean(vals) for key, vals in tl_events_pred_current.items()}
    else:
        tl_events_pred_current = None
    ...
    for agent, agent_class in agents_with_wheels:
        ...
        if find_closest_result is not None:
            ...
            if lane_id in controlled_lane_id_2_tl_signal_idx:
                ...
                if tl_events_pred_current is not None and ego_intersection_i == intersection_i:
                    ...
                    tl_tte_25th_perc = tl_events_pred_current[f'{tl_signal_idx}_tte_25th_perc']
                    tl_tte_75th_perc = tl_events_pred_current[f'{tl_signal_idx}_tte_75th_perc']
                else:
    ...
    for lane_info_entry in track_speed_yaw_lane_point_list:
        (agent_centroid_x, agent_centroid_y, _, _, _,
