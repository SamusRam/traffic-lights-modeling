...
def get_dataloader(tl_events_df, intersection_idx, shuffle=True, batch_size=BATCH_SIZE, num_workers=NUM_WORKERS):
...
def train_intersection_model(intersection_idx, device, lr=3e-4, embedding_dim=64, hidden_dim=64, n_layers=2, 
    ...
    early_stopping = EarlyStopping(patience=5, verbose=True, path=f'{output_name}_intersection_{intersection_idx}_combined_loss_checkpoint.pt')
